...
const markAttendance = async (req, res) => {
  ...
  try {
    console.log(
      `📍 Marking attendance: ${employeeCode} - ${type} at ${timestamp}`,
    );

    // Find employee by employeeCode OR biometricId (for ZKTeco)
    const employee = await Employee.findOne({
      $or: [{ employeeCode }, { biometricId: employeeCode }],
    });
    ...
    const bangladeshPunchTime = new Date(
      punchTime.toLocaleString("en-US", { timeZone: "Asia/Dhaka" }),
    );
    const attendanceDate = getBangladeshDate(bangladeshPunchTime);

    console.log(
      `🕐 Punch time (Bangladesh): ${bangladeshPunchTime.toLocaleString("en-US", { timeZone: "Asia/Dhaka" })}`,
    );
    console.log(
      `📅 Attendance date (Bangladesh): ${attendanceDate.toLocaleDateString("en-US", { timeZone: "Asia/Dhaka" })}`,
    );

    // ✅ Check if this is an off-day (weekend or holiday)
    const isOffDay =
      isWeekend(attendanceDate) || (await isHoliday(attendanceDate));
    ...
    if (isOffDay) {
      ...
      if (
        firstInPunch &&
        type === "IN" &&
        attendance.punches.filter((p) => p.type === "IN").length === 1
      ) {
        ...
        // ✅ NEW: Half day time (11:00 AM)
        const halfDayDate = new Date(bangladeshPunchTime);
        halfDayDate.setHours(11, 0, 0, 0);
        ...
        console.log(
          `⏰ Shift start: ${shiftStartTime.toLocaleTimeString()}, In time: ${inTime.toLocaleTimeString()}`,
        );
        console.log(
          `⏱️ Minutes before/after shift: ${minutesBeforeShift} / ${minutesAfterShift}`,
        );

        // ✅ NEW: Check if punch is after 11 AM (auto half day)
        if (inTime >= halfDayTime) {
          attendance.status = "Present";
          attendance.lateMinutes = minutesAfterShift;
          attendance.isHalfDay = true;
          attendance.timingStatus = "Half Day";
          console.log(
            `⚠️ HALF DAY: Punched at ${inTime.toLocaleTimeString()} (after 11:00 AM)`,
          );
        } else if (inTime < shiftStartTime) {
          attendance.status = "Present";
          attendance.lateMinutes = 0;
      ...
      if (lastOutPunch) {
        ...
        console.log(
          `⏰ Shift end: ${shiftEndTime.toLocaleTimeString()}, Out time: ${outTime.toLocaleTimeString()}`,
        );
    ...
    console.log(
      `✅ Attendance saved - Date: ${attendanceDate.toLocaleDateString()}, Total breaks: ${attendance.totalBreakMinutes} min, Net working: ${attendance.netWorkingMinutes} min`,
    );
    ...
    res.status(200).json({
      ...
      bangladeshTime: bangladeshPunchTime.toLocaleString("en-US", {
        timeZone: "Asia/Dhaka",
      }),
    });
  } catch (error) {
...
const getAttendance = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.find(query)
      .populate(
        "employee",
        "firstName lastName employeeCode shiftStart shiftEnd",
      )
      .sort({ date: -1 });
...
const manualAttendance = async (req, res) => {
  try {
    ...
    if (attendance.isLocked) {
      return res
        .status(400)
        .json({ message: "Attendance is locked for payroll" });
    }
    ...
    if (inTime) {
      const bangladeshInTime = new Date(
        new Date(inTime).toLocaleString("en-US", { timeZone: "Asia/Dhaka" }),
      );
      attendance.inTime = bangladeshInTime;
    }

    if (outTime) {
      const bangladeshOutTime = new Date(
        new Date(outTime).toLocaleString("en-US", { timeZone: "Asia/Dhaka" }),
      );
      ...
      if (bangladeshOutTime > shiftEndTime) {
        attendance.overtimeMinutes = getMinutesDifference(
          bangladeshOutTime,
          shiftEndTime,
        );
        attendance.hasOvertime = true;
        ...
        attendance.earlyLeaveMinutes = getMinutesDifference(
          shiftEndTime,
          bangladeshOutTime,
        );
        attendance.earlyLeave = true;
...
// @desc    Get MY attendance records (for the logged-in user)
...
const getMyAttendance = async (req, res) => {
  try {
    ...
    console.log(
      `📋 Getting my attendance for user: ${req.user.name} (${req.user.role})`,
    );
    ...
    if (!user.employeeId) {
      console.log("⚠️ No employee record linked to this user");
